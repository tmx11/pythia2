...
begin
  ...
  try
    try
      ...
      // Check if Pythia action already exists in IDE ActionList (prevent duplicates)
      ActionList := NTAServices.ActionList;
      AlreadyExists := False;
      
      if Assigned(ActionList) then
      begin
        LogLines.Add('Checking IDE ActionList for existing Pythia actions...');
        LogLines.Add('Total actions in IDE: ' + IntToStr(ActionList.ActionCount));
        
        for J := 0 to ActionList.ActionCount - 1 do
        begin
          // Check Name for Pythia (Caption property only in TAction, not TBasicAction)
          if Assigned(ActionList.Actions[J]) and 
             (Pos('Pythia', ActionList.Actions[J].Name) > 0) then
          begin
            LogLines.Add('Found existing Pythia action: Name="' + ActionList.Actions[J].Name + '"');
            AlreadyExists := True;
            Break;
          end;
        end;
      ...
      begin
        LogLines.Add('Pythia action already exists in IDE ActionList');
        LogLines.Add('Skipping duplicate menu creation');
        LogLines.Add('SUCCESS: Menu already registered');
        goto SkipCreation;
      end;
      
      LogLines.Add('No existing Pythia action found - proceeding with creation');
      
      // Create menu handler
      if not Assigned(MenuHandler) then
        MenuHandler := TPythiaMenuHandler.Create;
      
      // Create Pythia Chat menu item (but don't add to ToolsMenu directly)
      MenuItemChat := TMenuItem.Create(nil);
      ...
      // AddActionMenu handles both action registration AND menu insertion - don't use ToolsMenu.Add()
      if Supports(BorlandIDEServices, INTAServices, NTAServices) then
      begin
        NTAServices.AddActionMenu('ToolsMenu', nil, MenuItemChat);
        LogLines.Add('AddActionMenu called successfully - menu item added once');
      end;
